...
async def _send_detailed_help(update: Update, context: ContextTypes.DEFAULT_TYPE, cmd_name: str):
    ...
    if not info:
        # Message d'erreur
        error_message = f"⚠️ 命令 `/{cmd_name}` 未找到。请使用 `/help` 查看完整列表。"
        await update.message.reply_text(
            error_message,
            parse_mode=ParseMode.MARKDOWN_V2,
            reply_markup=get_main_menu_keyboard()  # <-- ATTACHER LE MENU
    ...
    message = f"📚 **详细帮助：/\{cmd_name}** 📚\n"
    message += f"**用法 (Usage):** `{safe_usage}`\n"
    message += f"**描述 (Description):** {safe_long_desc}"
